//
// Created by cosmosmount on 2025/9/13.
//
 ...
 * @param canId 电机ID
 */
void DJIMotorHandler::registerMotor(DJIMotor *DJImotor, FDCAN_HandleTypeDef *hfdcan, uint16_t canId)
{
    DJImotor->canId = canId;
    DJImotor->hcan = hfdcan;

    if (canId >= 0x201 && canId <= 0x208)
    {
        if (DJImotor->hcan == &hfdcan1)
        {
 ...
 * @param rx_data
 */
void DJIMotorHandler::updateFeedback(FDCAN_HandleTypeDef *hfdcan, uint8_t *rx_data, int index)
{
    if (hfdcan == &hfdcan1)
    {
        // 处理CAN1的电机
...
void DJIMotorHandler::UpdateSensorData(DJIMotor *motor, uint8_t *can_data)
{
    ...
    motor->motorFeedback.currentFdb = (int16_t)(can_data[4] << 8 | can_data[5]);
