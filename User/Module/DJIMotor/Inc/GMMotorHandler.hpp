...
public:
    /**
     *@brief 使用指针数组, 3个CAN口，每个CAN口最多8个电机
     */
    GMMotor *GMMotorList[3][8];
    ...
    uint8_t can3_send_data_0[8]; // CAN3电机控制数据，用于控制0x201-0x204
    uint8_t can3_send_data_1[8]; // CAN3电机控制数据，用于控制0x205-0x208
    ...
    bool CAN3_0x200_Exist = false; // CAN3是否存在控制报文为0x200电机
    bool CAN3_0x1FF_Exist = false; // CAN3是否存在控制报文为0x1FF电机
     ...
     * @param canId 电机ID
     */
    void registerMotor(GMMotor *GMmotor, FDCAN_HandleTypeDef *hcan, uint16_t canId); // 使用指针作为参数
     ...
     */
    //void processRawData(FDCAN_HandleTypeDef *hcan, uint8_t *rx_data, int index);
     ...
     * @param hcan2 CAN2句柄
     */
    void updateFeedback(FDCAN_HandleTypeDef *hcan, uint8_t *rx_data, int index);

    void Receive(GMMotor *motor, uint8_t *can_receive_data);
