...
void Process_Host_Request(const msg_hostreq_t& req, const msg_launcher2sysctrl_t& lch2sys,
                            DartLibrary* dart, om_topic_t* hosttx_topic);
...
[[nonreturn]] void SysctrlThreadFun(ULONG initial_input)
{
    ...
    om_suber_t *hostreq_suber = nullptr;
    om_topic_t *hosttx_topic = nullptr;
    msg_hostreq_t host_req{};
    ...
    for (;;)
    {
        ...
        if (hostreq_suber == nullptr)
        {
            om_topic_t* hostreq_topic = om_find_topic("hostreq", 0);
            if (hostreq_topic != nullptr)
            {
                hostreq_suber = om_subscribe(hostreq_topic);
            }
        }
        if (hosttx_topic == nullptr)
        {
            hosttx_topic = om_find_topic("hosttx", 0);
        }
        ...
        if (hostreq_suber != nullptr &&
            hosttx_topic != nullptr &&
            om_suber_export(hostreq_suber, &host_req, false) == OM_OK)
        {
            Process_Host_Request(host_req, lch2sys, &dart_lib, hosttx_topic);
        }
 ...
 * @param cmd 
 */
static constexpr float HOST_YAW_MIN = -5.0f;
static constexpr float HOST_YAW_MAX = 5.0f;
static constexpr float HOST_TENSION_MIN = 0.0f;
static constexpr float HOST_TENSION_MAX = 120.0f;
static constexpr float HOST_PRE_TENSION_MIN = 0.0f;
static constexpr float HOST_PRE_TENSION_MAX = 120.0f;
...
void Process_Host_Request(const msg_hostreq_t& req, const msg_launcher2sysctrl_t& lch2sys,
                            DartLibrary* dart, om_topic_t* hosttx_topic)
{
    if (!req.valid || dart == nullptr || hosttx_topic == nullptr)
    {
        return;
    }
    ...
    switch (req.kind)
    {
        case HOST_REQ_HEARTBEAT:
            HostSendHeartbeat(hosttx_topic, *dart, req.seq);
            break;

        case HOST_REQ_GET_DART_TABLE:
            for (uint8_t id = 1; id <= 16; id++)
            {
                HostSendDartParamState(hosttx_topic, *dart, req.seq, id);
            }
            HostSendAck(hosttx_topic, req.type, req.seq, HOST_ACK_APPLIED, dart->config.config_revision);
            break;
        ...
        case HOST_REQ_SET_DART_PARAMS_BATCH:
        {
            if (req.target_type > 1 ||
                ...
                req.dart_count > 8)
            {
                HostSendError(hosttx_topic, req.type, req.seq, HOST_ERR_INVALID_COMMAND, req.update_mask);
                break;
            }
            ...
            {
                HostSendError(hosttx_topic, req.type, req.seq, HOST_ERR_PARAM_OUT_OF_RANGE, 1);
                break;
            }
            ...
            {
                HostSendError(hosttx_topic, req.type, req.seq, HOST_ERR_PARAM_OUT_OF_RANGE, 2);
                break;
            }
            ...
            if (!valid_ids)
            {
                HostSendError(hosttx_topic, req.type, req.seq, HOST_ERR_PARAM_OUT_OF_RANGE, 3);
                break;
            }
            if (duplicate)
            {
                HostSendError(hosttx_topic, req.type, req.seq, HOST_ERR_DUPLICATE_DART_ID, 0);
                break;
            }
            ...
            for (uint8_t i = 0; i < req.dart_count; i++)
            {
                HostSendDartParamState(hosttx_topic, *dart, req.seq, req.dart_ids[i]);
            }
            break;
        }

        case HOST_REQ_GET_SEQUENCE:
            HostSendSequenceState(hosttx_topic, *dart, req.seq);
            break;
        ...
        case HOST_REQ_SET_SEQUENCE:
            for (uint8_t i = 0; i < 4; i++)
            {
                ...
                {
                    HostSendError(hosttx_topic, req.type, req.seq, HOST_ERR_PARAM_OUT_OF_RANGE, i);
                    return;
                }
            }
            for (uint8_t i = 0; i < 4; i++)
        ...
        case HOST_REQ_GET_PRE_TENSION:
            HostSendPreTensionState(hosttx_topic, *dart, req.seq);
            break;

        case HOST_REQ_SET_PRE_TENSION:
            if (!HostFloatInRange(req.pre_tension, HOST_PRE_TENSION_MIN, HOST_PRE_TENSION_MAX))
            {
                HostSendError(hosttx_topic, req.type, req.seq, HOST_ERR_PARAM_OUT_OF_RANGE, 0);
                break;
            }
        ...
        default:
            HostSendError(hosttx_topic, req.type, req.seq, HOST_ERR_UNKNOWN_TYPE, req.kind);
            break;
    }
}
