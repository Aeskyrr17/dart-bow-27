...
[[nonreturn]] void SensorThreadFun(ULONG initial_input) 
{
    ...
    hall.L_reset = false;
    hall.R_reset = false;
    msg_sensor.fire_done = false;
    msg_sensor.coil_reset = false;
    msg_sensor.door_open = false;
    msg_sensor.string_tight = false;
    ...
    for (;;)
    {
        ...
        if (HAL_GPIO_ReadPin(LIGHT_PORT, LIGHT_PIN))
            msg_sensor.launchplat_return = true;
        else 
            msg_sensor.launchplat_return = false;

        if (hall.L_reset && hall.R_reset)
            msg_sensor.coil_reset = true;
        else 
            msg_sensor.coil_reset = false;
